 ...
 *
 * - Studio mode: always open (multi-tenant signups are the whole point).
 * - Local mode: only open if STDOUT_ALLOW_SIGNUPS=true. Default: closed.
 *   (The setup wizard creates the first user regardless of this flag.)
 */
export function publicSignupsAllowed(): boolean {
  if (IS_STUDIO) return true;
